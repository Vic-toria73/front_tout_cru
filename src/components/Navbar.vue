      ...
      <Transition name="slide">
         ...
         <div v-if="isAuthenticated">
            ...
            <button @click="onLogout"
              class="h-12 px-4 hover:bg-[#F90404]/100 rounded-xl transition">Déconnexion</button>
          </div>
          
          <div v-else class="fixed top-0 right-0 h-full w-64 bg-primary shadow-lg z-50 p-6 
         flex flex-col justify-center gap-4">
            <router-link class="flex items-center h-12 px-4 hover:bg-background-alt/100 rounded-xl transition" to="/login">Connexion</router-link>
            <router-link class="flex items-center h-12 px-4 hover:bg-background-alt/100 rounded-xl transition" to="/register">Inscription</router-link>
          </div>
        </div>
      </Transition>
